...
is_title_visible = False   # Flag to control showing the title
...
class Game:
    ...
    def game_over(self):
        global is_title_visible
        self.snake.reset()
        self.food.position = self.food.generate_random_pos(self.snake.body)
        self.state = "STOPPED"
        is_title_visible = True # Make the flag true
...
while True:
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            if game.state == "STOPPED":
                game.state = "RUNNING"
                is_title_visible = False    #If game is running make the title invisible
            elif game.state == "RUNNING":  # Only process movement keys when running
    ...
    if is_title_visible:
        ...
        screen.blit(title_surface, (70, 20))
